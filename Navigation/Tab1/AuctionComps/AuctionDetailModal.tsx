...
import Product from "../../../components/OrderImage/Product"; // Assuming Product is the correct component to show order details
import { BASE_URL } from "../../../ReuseComponent/Env"; // Ensure BASE_URL is correct
import OrderDetailSkeleton1 from "../OrderComps/OrderDetailSkeleton";
...
const AuctionDetailModal = ({ closeModal, visible, bookingId, filename }) => {
  ...
  const handleDetailPress = async () => {
    ...
    try {
      ...
      formdata.append("auction_id", bookingId.toString()); // Ensure bookingId is a string

      const response = await fetch(`${BASE_URL}/auctionOrder_api.php?action=select_id`, {
        method: "POST",
        headers: {
