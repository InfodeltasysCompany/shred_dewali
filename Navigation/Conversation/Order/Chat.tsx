...
//         GChatstate.currentConversationData.prodDetails && 
//         GChatstate.currentConversationData.prodDetails.firebase_pid &&
//         GChatstate.currentConversationData.sellerDetails &&
//         GChatstate.currentConversationData.buyerDetails
//       ) {
//         // Extract necessary data from the current conversation state
//         const { prodDetails, sellerDetails, buyerDetails } = GChatstate.currentConversationData;
//         const { firebase_pid } = prodDetails; // Product ID from the current conversation
//         const buyerId = buyerDetails.firebase_uid; // Firebase UID for the buyer
//         const sellerId = sellerDetails.firebase_uid; // Firebase UID for the seller
//         const senderID = state.f_id; // Sender ID from the state (current user)

//         console.log("firebase prod id is :", firebase_pid);
...
//         // Construct the conversation ID based on product and user IDs
//         const conversationID = `${prodDetails.productId}_seller_${sellerId}_buyer_${buyerId}`;

//         // Call the sendMessage function to send the message
//         sendMessage(firebase_pid,state.f_id,firebase_pid,message,state.f_id);
//         // sendMessage(firebase_pid, buyerId, sellerId, message, senderID);
//       } else {
...
const Chat = () => {
  ...
  const handleSend = async () => {
    if (message.trim()) {
      ...
      // Check if all necessary data exists in the current conversation state
      if (
        GChatstate.currentConversationData &&
        GChatstate.currentConversationData.prodDetails && 
        GChatstate.currentConversationData.prodDetails.firebase_pid &&
        ...
        const { prodDetails, sellerDetails, buyerDetails } = GChatstate.currentConversationData;
        const { firebase_pid,productId } = prodDetails; // Product ID from the current conversation
        const buyerId = state.f_id; // Firebase UID for the buyer
        const sellerId = prodDetails.productId; // Firebase UID for the seller
        const senderID = state.f_id; // Sender ID from the state (current user)
        ...
        await sendMessage(productId, senderID, firebase_pid, message, senderID);
      } else {
